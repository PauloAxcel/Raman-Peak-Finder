...
import analysisFunctions_v11 as af
    ...
    def runPlot(self):
        ...
        if not inputFilePath:
            ...
            for file in inputFilePath: 
                ...
                wavenumber, intensity, n_points, n_raman,label, flag = af.ImportData(file,zscore,inv)  
...
#        print(text)
    ...
    def runStackPlot(self):
        ...
        if not inputFilePath:
            ...
            if self.baselineCheckBox.isChecked() and self.stdCheckBox.isChecked() and self.normalizationCheckBox.isChecked():
                wavenumber,z,label,sstd = af.spec(inputFilePath,zscore,inv)
        ...
        if not inputFilePath:         
            self.nofile_warning_applications()
             ...
             if self.LoadingsCheckBox.isChecked() and self.GaussianCheckBox.isChecked() and self.DistanceCheckBox.isChecked():
                wavenumber,dataset,target,flag = af.sortData(inputFilePath,asymmetry_param, smoothness_param, max_iters, conv_thresh,z,inv)
                if flag == 0:
