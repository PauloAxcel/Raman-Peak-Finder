...
def DistanceBarPlot2D(finalDf,targets,stylestring):
    ...
    if stylestring == 'binary':
        ...
        for n in range(len(list(np.unique(targets)))//param):
            ...
            for i in range(df_substrate.shape[0]):
                dist_substrate.append([np.sqrt(df_substrate.iloc[i,0]**2 + df_substrate.iloc[i,1]**2), df_substrate.iloc[i,-1]])
                dist_sub.append([np.sqrt(df_substrate.iloc[i,0]**2 + df_substrate.iloc[i,1]**2), df_substrate.iloc[i,-1]])

            
            for i in range(df_sugars.shape[0]):
                dist_substrate.append([np.sqrt(df_sugars.iloc[i,0]**2 + df_sugars.iloc[i,1]**2),df_sugars.iloc[i,-1]])
                dist_sug.append([np.sqrt(df_sugars.iloc[i,0]**2 + df_sugars.iloc[i,1]**2),df_sugars.iloc[i,-1]])
            ...
            for i in range(dist_sug.shape[0]):
                    for j in range(dist_sub.shape[0]):
                        dist_matrix.append([abs(dist_sug.iloc[i,0]-dist_sub.iloc[j,0]),dist_sub.iloc[j,1][:11] + ' - ' + dist_sug.iloc[i,1][:11]])
        ...
        for n in range(len(list(np.unique(targets)))//param):
            ...
            for i in range(df_other.shape[0]):
                dist_substrate.append([np.sqrt(df_other.iloc[i,0]**2 + df_other.iloc[i,1]**2),df_other.iloc[i,-1]])
                dist_oth.append([np.sqrt(df_other.iloc[i,0]**2 + df_other.iloc[i,1]**2),df_other.iloc[i,-1]])
            ...
            for i in range(dist_oth.shape[0]):
                for j in range(dist_sub.shape[0]):
                    dist_matrix.append([abs(dist_oth.iloc[i,0]-dist_sub.iloc[j,0]),dist_sub.iloc[j,1][:11] + ' - ' + dist_oth.iloc[i,1][:11]])

            for i in range(dist_oth.shape[0]):
                for j in range(dist_sug.shape[0]):
                    dist_matrix.append([abs(dist_oth.iloc[i,0]-dist_sug.iloc[j,0]),dist_sug.iloc[j,1][:11] + ' - ' + dist_oth.iloc[i,1][:11]])
            ...
            for k in range(len(list(np.unique(targets)))//param):
                if n == 0 and k == 0:
                    ...
                    for i in range(dist_sub.shape[0]):
                        for j in range(dist_sub.shape[0]):
                            dist_base.append([abs(dist_sub.iloc[i,0]-dist_sub.iloc[j,0]),dist_sub.iloc[j,1][:11]])
                elif n >= k:
                    pass
...
#            dfdiff = pd.DataFrame(dist_matrix).iloc[:,1].unique()[:len(list(np.unique(targets)))//param-1][-1]
            
        #for the one pd.DataFrame(dist_matrix).iloc[:,1].unique()[:len(list(np.unique(targets)))//param-1]
...
#        box_pairs.append((names[n*param],names[n*param+1]))
    ...
    ax.set(ylim=(0,5))
    plt.title('Distance from origin to cluster')
...
#    plt.figure(figsize=(9,9/1.618))
#    plt.title('Distance from origin to cluster density plot')
#    ax2 = sns.histplot(dfmelezitose,x="Distance",hue="Substrate",element="poly",palette=colours,label=names)
#    h,l = ax2.get_legend_handles_labels()
#    ax2.legend(handles=h, labels=list(names), ncol = len(names)//6+1)
    ...
    ax3 = sns.barplot(x="Substrate", y="Distance", data= dfdiff ,ci="sd",capsize=.2,palette=colours2)
    ...
    if stylestring == 'binary' or stylestring == 'trinary' or stylestring == 'inter':
        ...
        for pair in box_pairs2:
            ...
            pvalues.append(p)
            print("pvalues:", pvalues)
        add_stat_annotation(ax3, data= dfdiff, x=x, y=y,
...
#    sns.lineplot(data = avgdfdiff['Avg Distance'], ax=ax3,color='r')
    ...
    ax5 = sns.histplot(dfdiff,x="Distance",hue="Substrate",element="poly",palette=colours2,label=names2)
    h,l = ax5.get_legend_handles_labels()
    ax5.legend(handles=h, labels=list(names2), ncol = len(names2)//6+1)
...
def DistanceBarPlot3D(finalDf,targets,stylestring):
    if stylestring == 'binary':
        ...
        for n in range(len(list(np.unique(targets)))//param):
            ...
            for i in range(df_substrate.shape[0]):
                dist_substrate.append([np.sqrt(df_substrate.iloc[i,0]**2 + df_substrate.iloc[i,1]**2 + df_substrate.iloc[i,2]**2), df_substrate.iloc[i,-1]])
                dist_sub.append([np.sqrt(df_substrate.iloc[i,0]**2 + df_substrate.iloc[i,1]**2 + df_substrate.iloc[i,2]**2), df_substrate.iloc[i,-1]])

            
            for i in range(df_sugars.shape[0]):
                dist_substrate.append([np.sqrt(df_sugars.iloc[i,0]**2 + df_sugars.iloc[i,1]**2 + df_sugars.iloc[i,2]**2),df_sugars.iloc[i,-1]])
                dist_sug.append([np.sqrt(df_sugars.iloc[i,0]**2 + df_sugars.iloc[i,1]**2 + df_sugars.iloc[i,2]**2),df_sugars.iloc[i,-1]])
        ...
        for n in range(len(list(np.unique(targets)))//param):
            ...
            for i in range(df_other.shape[0]):
                dist_substrate.append([np.sqrt(df_other.iloc[i,0]**2 + df_other.iloc[i,1]**2 + df_other.iloc[i,2]**2),df_other.iloc[i,-1]])
                dist_oth.append([np.sqrt(df_other.iloc[i,0]**2 + df_other.iloc[i,1]**2 + df_other.iloc[i,2]**2),df_other.iloc[i,-1]])
