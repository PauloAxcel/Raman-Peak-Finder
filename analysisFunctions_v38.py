...
#files = [r'C:\Users\paulo\OneDrive - University of Birmingham\Desktop\birmingham_02\saliva\saliva data\milk\saliva_785nm_static1150_3s_3acc_10%_50x_map24_toothpickv02.txt',
...
#inv=0  
#    
#keygen = [r'C:\Users\paulo\OneDrive - University of Birmingham\Desktop\birmingham_02\saliva\saliva data\milk\saliva_785nm_static1150_3s_3acc_10%_50x_map24_toothpickv02.txt',
...
def NeuronActivationperPlot(som, data,smooth):   
...
#    fig,ax = plt.subplots(neuron,neuron,sharex=True,sharey=True)
#    axes = ax.T.ravel()
    ...
    for i in range(properties.shape[0]):
        ...
        if math.isnan(properties.iloc[i,2:].values[0]):
            ...
            rect2 = patches.Rectangle((patchx+l/3, patchy) , l/3 , w, edgecolor='k', facecolor='none')
            rect3 = patches.Rectangle((patchx, patchy+w/3) , l , w/3, edgecolor='k', facecolor='none')
            
            r = properties['row'].loc[i]
            c = properties['col'].loc[i]
    
            patch = patches.Rectangle((patchx+r*l/3, patchy+c*w/3),l/3 , w/3, edgecolor='k', facecolor='k')
            ax.add_patch(patch)
            ...
            for wav in wavenumber[local_max_index]:
                ...
                try:
                    ...
                    if (abs(pars_1[-1])>100) | (pars_1[0]<0):
                        ...
                        lorentz.append(_1Lorentzian(wavenumber,*popt_1lorentz))
                        peak_vector.append([label+' ('+str(int(r))+' '+str(int(c))+' )']+popt_1lorentz.tolist()+['major'])
                        err.append(perr_1lorentz[0])
            ...
            if (l<k+tol) & (k-tol<l):
                ...
                if (df1.columns[-1] == 'Intensity (a.u.)') & (df2.columns[-1] == 'Intensity (a.u.)'):
                    dataset.append([np.mean([k,l]),
                                ...
                                df1['Intensity (a.u.)'][df1['center']==k].values[0]/df2['Intensity (a.u.)'][df2['center']==l].values[0],
                                (df1['err'][df1['center']==k].values[0]*df2['Intensity (a.u.)'][df2['center']==l].values[0]-df1['Intensity (a.u.)'][df1['center']==k].values[0]*df2['err'][df2['center']==l].values[0])/(df2['Intensity (a.u.)'][df2['center']==l].values[0])**2])
...
def FilePeakMatching(file1,file2,tol):
    if len(file1) ==1 & len(file2) ==1:
        ...
        while i<2:
            ...
            name1 = file1[0].split('/')[-1]
            name2 = file2[0].split('/')[-1]
            
            newdf1,newdf2 = cleanfilepeak([df1,df2])
            data = comparefiles([newdf1,newdf2],tol)
            pd.DataFrame(data,columns=['center','label','ratio','err']).to_csv(name1+'_'+name2+'.csv',index=False)
