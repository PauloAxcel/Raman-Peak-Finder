...
import analysisFunctions_v52 as af  
...
class MyApp(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.NeuParamBox.setText("5")
        self.numberParamBox.setText("5")
            ...
            for file in inputFilePath: 
                ...
                if self.baselineCheckBox.isChecked() and self.stdCheckBox.isChecked() and self.normalizationCheckBox.isChecked():
                    ...
                    if flag == 0 or intensity.shape[1]<1:
                        ...
                        af.MultipleIndividualPlot(wavenumber,intensity,label)
                        if n == 0 and self.HelperCheckBox.isChecked():
    ...
    #            wavenumber, intensity, n_points, n_raman,label = af.ImportData(file)  
            ...
            if self.baselineCheckBox.isChecked() and self.stdCheckBox.isChecked() and self.normalizationCheckBox.isChecked():
                ...
                if windowmin != 0 and windowmax !=0:
                    af.ThreeDPlotrange(wavenumber,spectra,empty,label, windowmin, windowmax,ax)
                else:
                    af.ThreeDPlot(wavenumber,spectra,empty,label,ax)
...
#                 self.resultsBox.setText(' '.join(info+[' '.join(val) for val in distance]))
    ...
    def PeakFinder(self):
        ...
        neuron = int(self.NeuParamBox.text())
...
#            peak,spectra = af.peakfinder(keygen,asymmetry_param,smoothness_param,max_iters,conv_thresh,zscore,inv)
            peak,spectra = af.peakfinder(keygen,smooth,asymmetry_param,smoothness_param,max_iters,conv_thresh)
  ...
  #input a lock and a key lock ie something you want to find out and a key ie a known solution with peaks
        ...
        if keygen and lockgen:
            if self.LoadingsCheckBox_2.isChecked() and self.matchCheckBox.isChecked():
                ...
                # if flag == 1:
                #     self.singlefile_warning_applications() 
...
#            af.comparison(keygen,lockgen)
    ...
    def selectFile(self):
        ...
        if self.FolderCheckBox.isChecked():
            openFile = QtWidgets.QFileDialog.getExistingDirectory(self, 'Select project folder:', "", QtWidgets.QFileDialog.ShowDirsOnly)
            classes, subdires,files= af.listdirs(openFile)
            classes = [c for c in classes if len(c.split('.'))==1]
            # print(files)
            openFile = af.cleanfile(files)
            openFile = af.converttxt(openFile,classes)
            # print(openFile)
...
#        print(dir_)
    ...
    def singlefile_warning_applications(self):
        QMessageBox.warning(self,'Use single .csv files',
                                  ".",
                                  QMessageBox.Ok)
...
#                                  QMessageBox.Ok)
    ...
    def lab_warning_applications(self):
        QMessageBox.warning(self,'Label Error',
                                  "Make sure the concentration used is on the first 8 characters which includes the units" ,
                                  QMessageBox.Ok)
