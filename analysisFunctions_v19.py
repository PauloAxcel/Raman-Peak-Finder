...
#        plt.figure()
        ...
        peak.to_csv('loadings'+'Q'+str(Q)+df['sample'][0]+'.csv',sep=';', index=False)
...
lockgen = r'C:\Users\paulo\OneDrive - University of Birmingham\Desktop\birmingham_02\saliva\GUI for raman analysis\8)80nm_mel_.csv'
...
#                 
#     m = list(set(match))
#     cluster_m = cluster(m,5)
#    
#     dataset = []
#     
#     for c in cluster_m:
#         dataset.append(np.mean(c))
#        
#        
#     real_lock = lock[lock['label'] == lock['label'].iloc[-1]]
#     wave = np.linspace(real_lock['center'].min()*1.1,real_lock['center'].max()*1.1,1000)
#     sum_lor = []
#    
#     for cent,inten,wid in zip(real_lock['center'].tolist(),real_lock['Intensity (a.u.)'].tolist(),real_lock['width'].tolist()):
#         sum_lor.append(_1Lorentzian(wave,inten,cent,wid))
#    
#     sum_lor = pd.DataFrame(sum_lor).sum()
#    
#    
#    
#    
...
#    
#        index2 = df['Intensity (a.u.)'] >0
#        index3 = (wave.min()<=df['center']) & (df['center']<=wave.max())
#        
#        
#        
#        index4 = df['importance'] == 'major'
...
#        
#     for data in dataset:
#         plt.plot(wave,sum_lor)
#         plt.fill_between(np.linspace(data-5,data+5,10),sum_lor.max(),color='y',alpha=0.2)
#        
#        
#
#
