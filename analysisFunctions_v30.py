...
#        colours.append(cycle[i%len(cycle)])
    ...
    if stylestring == 'binary':
        ...
        param = 2
        fig, axs = plt.subplots(math.ceil(len(list(np.unique(targets)))//param/3), 3 ,figsize=(5*len(list(np.unique(targets)))//param,5))
        fig2, axs2 = plt.subplots(math.ceil(len(list(np.unique(targets)))//param/3), 3 ,figsize=(5*len(list(np.unique(targets)))//param,5))
        fig3, axs3 = plt.subplots(math.ceil(len(list(np.unique(targets)))//param/3), 3 ,figsize=(5*len(list(np.unique(targets)))//param,5))
...
#maps = [r'C:\Users\paulo\OneDrive - University of Birmingham\Desktop\birmingham_02\own papers\paper 3\data\SADSAC_512map_static950_1sec_10%_1acc_785nm_pinhole_0111.txt',
#        r'C:\Users\paulo\OneDrive - University of Birmingham\Desktop\birmingham_02\own papers\paper 3\data\SADSACS2_512map_static950_1sec_10%_1acc_785nm_pinhole_0111.txt']
...
#    dist['prod'] = dist['dist']/dist['er']
    ...
    for i in range(dist_n):
        lab1 = dist['label0'].iloc[i]
        lab2 = dist['label1'].iloc[i]
        PC = [dist.iloc[i]['PCi'] , dist.iloc[i]['PCf']]
        ...
        pclab1 = ''.join([ s[0] for s in PC[0].split() ])
        pclab2 = ''.join([ s[0] for s in PC[1].split() ])
        ...
        loadings = pd.DataFrame(pca.components_.T * np.sqrt(pca.explained_variance_),
                                columns=['principal component '+ str(m+1) for m in range(pca.components_.T.shape[0])])
...
#                ax.remove()
        ...
        axbig.fill_between(np.linspace(-data0[0].max()*1.1,0,10),0,(data0[1].max()*1.1),alpha=0.2)
        axbig.fill_between(np.linspace(-data0[0].max()*1.1,0,10),0,(-data0[1].max()*1.1),alpha=0.2)
        axbig.fill_between(np.linspace(0, data0[0].max()*1.1, 10),0,(-data0[1].max()*1.1),alpha=0.2)
        axbig.legend(loc='best', frameon=False,prop={'size': 8})
        ...
        for m_1,m_2 in zip(m1,m2):
            ...
            local_max_index2 = (y_m>0) & (find_local_max(y_m))
            stem2 = np.zeros(x.shape)
            stem2[local_max_index2] = 1 

            combax[k].plot(x[y_m>0],y_m[y_m>0],'o')
            combax[k].plot(x[local_max_index2],y_m[local_max_index2],'rx')
            combax[k].text(x.min()*1.1,y_m.max()*0.7,'Q'+str(k+1),color=colours[k])
    
            combax[k].set_xlabel('Raman shift (cm$^{-1}$)')
            combax[k].set_ylabel('L(a.u.)')

            for wav in wavenumber[local_max_index2]:
                reg = fit_region(wavenumber,wav,10)
                
                if (y_m[reg[reg==wav].index][0] > 0):
                    try:
                        popt_1lorentz, pcov_1lorentz = scipy.optimize.curve_fit(_1Lorentzian, 
                                                                        reg, 
                                                                        y_m[reg.index],
                                                                        p0=[y_m[reg.index].max(), wav, 2/(np.pi*y_m[reg.index].max())])
                        pars_1 = popt_1lorentz[0:3]
                        combax[k].plot(wavenumber,_1Lorentzian(wavenumber,*popt_1lorentz),lw=2)
                        combax[k].fill_between(wavenumber, _1Lorentzian(wavenumber,*popt_1lorentz).min(),_1Lorentzian(wavenumber,*popt_1lorentz), alpha=0.5)
                        peaksQ.append([pclab1]+popt_1lorentz.tolist()+['Q'+str(k+1)])
    ...
    #    plt.grid(True)
        ...
        text.append([str(round(dist.iloc[i]['dist'],3))+' +/- '+
                     str(round(dist.iloc[i]['er'],3))+' '+pclab1+' - '+pclab2])
...
def peakmatching(keygens,lockgens,tol):
...
#keygens = [r'C:\Users\paulo\OneDrive - University of Birmingham\Desktop\birmingham_02\saliva\GUI for raman analysis\keymilk_785nm_.csv']
#lockgen = [r'C:\Users\paulo\OneDrive - University of Birmingham\Desktop\birmingham_02\saliva\GUI for raman analysis\loadingsall1st_saliva_.csv']
#    
