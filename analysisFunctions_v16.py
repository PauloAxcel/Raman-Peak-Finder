...
def peakfinder(files,asymmetry_param = 0.05,smoothness_param = 1000000,max_iters = 10,conv_thresh =0.00001,zscore=0,inv=0):
...
#        ax4.plot(wavenumber,-residual_1lorentz,'o')
        ...
        for wav in wavenumber[local_max_index2]:
            ...
            if (residual_1lorentz[reg[reg==wav].index].values[0] > 0):
...
#        ax2.plot(wavenumber, residual_1lorentz2,'o',color='g',alpha=0.4)
    ...
    peak.to_csv(label+'.csv',sep=';', index=False)
    
    if os.path.isfile('MASTER.csv'):
        peak.to_csv('MASTER.csv', sep=';', index=False,mode='a')
        clean_master('MASTER.csv')
    else:
        peak.to_csv('MASTER.csv', sep=';', index=False)
...
def plotprinc(df,loadings,wavenumber):
...
#    axbig.set_xticks(np.arange(df['principal component 1'].min()-1, df['principal component 1'].max()+1, (-df['principal component 1'].min()+df['principal component 1'].max()+2)))
#    axbig.set_yticks(np.arange(df['principal component 2'].min()-1, df['principal component 2'].max()+1, (-df['principal component 2'].min()+df['principal component 2'].max()+2)))
    ...
    axbig.fill_between(np.arange(0, df['principal component 1'].max()*1.1, (df['principal component 1'].max())/10),0,(df['principal component 2'].max()*1.1),alpha=0.2)
    axbig.fill_between(np.arange(-df['principal component 1'].max()*1.1,df['principal component 1'].max()*.1, (df['principal component 1'].max())/10),0,(df['principal component 2'].max()*1.1),alpha=0.2)
    axbig.fill_between(np.arange(0, df['principal component 1'].max()*1.1, (df['principal component 1'].max())/10),0,(-df['principal component 2'].max()*1.1),alpha=0.2)
    axbig.fill_between(np.arange(-df['principal component 1'].max()*1.1,df['principal component 1'].max()*.1, (df['principal component 1'].max())/10),0,(-df['principal component 2'].max()*1.1),alpha=0.2)
    axbig.legend(loc='best', frameon=False,prop={'size': 8})
    axbig.set_xlabel('PC1')
    axbig.set_ylabel('PC2')
...
def loadingpeak(maps,key,
                asymmetry_param, 
                smoothness_param, 
                max_iters, conv_thresh,
                zscore,inv,  windowmin=0,
                windowmax=0, dim=2,flag=1):
    ...
    plotprinc(df,loadings,wavenumber)
