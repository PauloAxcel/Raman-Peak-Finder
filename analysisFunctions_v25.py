...
#(X_train,y_train,X_test,y_test,len(class_names),random_state)
#
#n_classes = len(class_names)

#files = [r'C:\Users\paulo\OneDrive - University of Birmingham\Desktop\birmingham_02\Molecular Imprint surfaces\19_07_2019\SiAuDSAComp\S1.txt',
#         r'C:\Users\paulo\OneDrive - University of Birmingham\Desktop\birmingham_02\Molecular Imprint surfaces\19_07_2019\SiAuDSAComp\S2.txt',
#         r'C:\Users\paulo\OneDrive - University of Birmingham\Desktop\birmingham_02\Molecular Imprint surfaces\19_07_2019\SiAuDSAComp\S3.txt'       
#        ]

#dataset = norm
#
#def plot_roc_curve(dataset,labels):
#    
#    df_pca = pd.DataFrame(dataset).reset_index(drop=True)
#    
##    df_pca = df_pca.iloc[:,:840] 
#    
#    target = pd.DataFrame(labels,columns=['sample'])
#    
##    df_clean = []
##    target_clean = []
##    for targ in target['sample'].unique().tolist()[1::2]:
##        index = targ == target['sample']
##        for i in range(df_pca.loc[index].shape[0]):
##            df_clean.append(df_pca.loc[index].reset_index(drop=True).iloc[:,i])
##            target_clean.append(target.loc[index].reset_index(drop=True).iloc[i])
##    
##    df_clean = pd.DataFrame(df_clean).reset_index(drop=True)
##    target_clean = pd.DataFrame(target_clean).reset_index(drop=True)
##        
#    df_clean = df_pca
#    target_clean = target
#    random_state = 42
#    
#    X = df_clean.values
#    y = np.array(target_clean['sample'].values)
#    n_classes = len( target_clean['sample'].unique().tolist())
#    
#    y_n = label_binarize(y,classes= target_clean['sample'].unique().tolist())
#    # shuffle and split training and test sets
#    X_train, X_test, y_train, y_test = train_test_split(X, y_n, test_size=.2,  random_state=0)
#    
...
#    colours = bin_colours
    ...
    if stylestring == 'binary':
        ...
        fig2, axs2 = plt.subplots(len(list(np.unique(targets)))//param//3 if len(list(np.unique(targets)))//param>=3 else 1, 3 ,figsize=(5*len(list(np.unique(targets)))//param,5))
        fig3, axs3 = plt.subplots(len(list(np.unique(targets)))//param//3 if len(list(np.unique(targets)))//param>=3 else 1, 3 ,figsize=(5*len(list(np.unique(targets)))//param,5))
