...
def converttxt(openFiles,classes):
    ...
    for c in classes:
        ...
        files = [file for file in openFiles if file.split('/')[:-1][-2]==c]
        ...
        for cnt, file in enumerate(files):
                ...
                data.append([[0]*n_raman,[cnt]*n_raman,wavenumber,intensity])
            
        df = pd.DataFrame(np.hstack(data)).T
        df.to_csv(folder+'/'+c+'_pseudo_map.csv', index=False, header=False,sep='\t')
        newfiles.append(folder+'/'+c+'_pseudo_map.csv')
...
#        intensity.to_csv('/home/paulo/Documents/PhD/saliva/saliva data/all saliva beguining/saliva_all.csv')
...
#z = separate_stack(z)
#z,sstd = STDS(z)
#spectra = diff2(z,empty,z.std().mean())
#fig = plt.figure()
...
#
#inputFilePath = [r'C:\Users\paulo\OneDrive - University of Birmingham\Desktop\birmingham_02\MEng student\2nd year\MEng\RAMAN\gold coating\txt\65all_10mA_20s_633nm_1450static_map100_3sec_3acc_10%1.txt',
#                 r'C:\Users\paulo\OneDrive - University of Birmingham\Desktop\birmingham_02\MEng student\2nd year\MEng\RAMAN\gold coating\txt\108all_20mA_20s_633nm_1450static_map100_3sec_3acc_10%1.txt'
#                 ]
#
#wavenumber,z,label,sstd = spec(inputFilePath)
#z = separate_stack(z)
...
#ThreeDPlotrange(wavenumber,spectra,empty,label, windowmin, windowmax,ax)
#
#x = wavenumber
#y = spectra
#y2 = empty
      
def ThreeDPlotrange(x,y,y2,label, windowmin, windowmax,ax):
    ...
    if y2.empty:        
        ...
        if windowmin == 0 and windowmax ==0:
            pass
        elif windowmin == 0 and windowmax !=0:
            ranged = x<windowmax
        elif windowmax ==0 and windowmin != 0:
            ranged = x>windowmin  
        else:
            ranged = (x>windowmin)&(x<windowmax)
        ...
        for n in range(n_max):
            ...
            ax.view_init(30,-45)
            verts = [(x[i],y[i], z.values[i]) for i in range(z.shape[0])]+[(x.min(), y.min(),ax.axes.get_zlim()[0]),(x.max(), y.max(),ax.axes.get_zlim()[0])]
...
#file = r'/home/paulo/Documents/PhD/saliva/saliva data/milk/saliva_all.txt'
#wavenumber, intensity, n_points, n_raman,label, flag = ImportData(file)  

def MultipleIndividualPlot(x,y,label):
    NUM_COLORS = y.shape[1]
    cmaps = plt.get_cmap('gist_rainbow')
    colours = [cmaps(1.*i/NUM_COLORS) for i in range(NUM_COLORS)]
    ...
    for c,ym in zip(colours,sorted(y.max())):
        for ym2 in y.max().tolist():
            if ym2 == ym:
                plt.plot(x,y[y.max()[y.max()==ym].index.values].values,c=c)
...
#        residual_1lorentz2 = residual_1lorentz - lorentz_all2 
        ...
        ax2.plot(wavenumber, residual_1lorentz,'o',color='b',alpha=1)
...
def plotbands(dataset2):
    ...
    df = new_df['center']
    df.index = new_df['bond'].values
    c = []
    
    for lab in new_df['sample']:
        for co,l in zip(colours,new_df['sample'].unique()):
            if lab == l:
                c.append(co)
    ...
    xmin = new_df['center'].values.min()
    xmax=new_df['center'].values.max()
    reg = 50
    aa.set_xlim(xmin-xmin%reg,xmax+xmax%reg)
...
def listdirs(rootdir):
    ...
    for file in classes:
        ...
        if os.path.isdir(subdir):
            files.append(listdirs(subdir))
            
    return classes,subdires,files
