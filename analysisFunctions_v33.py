...
def ImportDataSOM(inputFilePath,windowmin,windowmax):
...
#            wavenumber = wavenumber[region]
        ...
        if (windowmax !=0) & (windowmin != 0):
            region = (xnew > windowmin) & (xnew < windowmax)
...
#            if (windowmax !=0) & (windowmin != 0):
#                ind_df = df['Intensity'][i*n_points:n_points*(i+1)].reset_index(drop=True).loc[region]
